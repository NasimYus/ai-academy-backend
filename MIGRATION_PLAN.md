...
- [ ] **F.4 i18n контента** — _рекомендация: MVP на одном языке, схему заложить под мультиязык позже_
- [ ] **F.5 Мультивалюта** — _рекомендация: MVP в одной валюте (TJS), абстракцию заложить позже_
...
- Платёжные шлюзы для MVP под рынок (Таджикистан): какие именно?
- Стратегия токенов: только access-JWT или access+refresh?
- Регистрация: оставляем многошаговую (`register/step/{step}`) или один шаг + дозаполнение профиля?
- Нужен ли OAuth (Google/FB) в MVP?
